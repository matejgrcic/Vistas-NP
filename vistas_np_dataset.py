...
transformation = tf.Compose([tf.Resize(image_size), tf.ToTensor()])
label_transformation = tf.Compose([tf.Resize(image_size, Image.NEAREST), tf.ToTensor()])
...
class VistasNPDataset(data.Dataset):
    n_classes = 66
    ...
    def get_crop_start(self, image_shape):
        [_, H, W] = image_shape
        start_h = 0
        start_w = 0
        if H > image_size:
            start_h = np.random.randint(0, H - image_size)
        if W > image_size:
            start_w = np.random.randint(0, W - image_size)
    ...
    def __getitem__(self, index):
        ...
        start_h, start_w = self.get_crop_start(image.size())
        image = image[:, start_h: start_h + image_size, start_w: start_w + image_size]
        label = label[:, start_h: start_h + image_size, start_w: start_w + image_size]
        return image, label
